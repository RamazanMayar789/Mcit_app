...
role="alert">
   <p>{{ $value }}</p>
    ...
    @endsession('success')
    <livewire:admin.create-students />
    <livewire:admin.edit-students />
    ...
    <tbody>
        @forelse ($students as $student)
            <tr class="border-t">
                ...
                <td class="px-1 py-1"><a href="{{ asset('storage/id_carts/' . $student->ID_Cart) }} " target="_blank"><img
                            src="{{ asset('storage/id_carts/' . $student->ID_Cart) }}" alt="کارت شناسایی" width="50" /></a></td>

                <td class="px-1 py-1"><img src="{{ asset('storage/students/' . $student->image) }}" alt="کارت شناسایی"
                        width="50" />
                </td>
                <td class="px-1 py-1 text-center">
                    <flux:button variant="outline" ><a href="{{ route('student.card.generate', [$student->id, 'jpg']) }}" class="  rounded  text-sm">چاپ کارت</a></flux:button>
...
<tr>
    ...
    <td colspan="8" class="px-1 py-1 text-center text-gray-600">
...
@include('livewire.admin.delete_modal')
</div>
